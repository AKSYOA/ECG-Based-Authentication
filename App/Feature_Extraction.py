...
def extract_ac_dct_non_fiducial(segments):
    features = []
    for i in range(len(segments)):
        acf = sm.tsa.acf(segments[i], nlags=2000)
        features.append(scipy.fftpack.dct(acf[0:400], type=2))
    return np.array(features)


def extract_wavelets_non_fiducial(segments):
    features = []
    for i in range(len(segments)):
        coffs = pywt.wavedec(segments[i], 'db4', level=4)
        features.append(coffs[0])
    return np.array(features)
